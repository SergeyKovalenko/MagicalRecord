...
- (NSManagedObject *)MR_findObjectForRelationship:(NSRelationshipDescription *)relationshipInfo
    ...
    if ([singleRelatedObjectData isKindOfClass:[NSString class]] || [singleRelatedObjectData isKindOfClass:[NSNumber class]]) {
...
- (void)MR_addObject:(NSManagedObject *)relatedObject forRelationship:(NSRelationshipDescription *)relationshipInfo {
    ...
    NSString *addRelatedObjectToSetMessage = [NSString stringWithFormat:addRelationMessageFormat,
                                                                        attributeNameFromString([relationshipInfo name])];
...
- (BOOL)MR_importValuesForKeysWithObject:(id)objectData {
    ...
    return [self MR_performDataImportFromObject:objectData
                              relationshipBlock:^(NSRelationshipDescription *relationshipInfo, id localObjectData) {
                                  ...
                                  SEL shouldImportSelector = NSSelectorFromString([NSString stringWithFormat:@"shouldImport%@:",
                                                                                                             [relationshipInfo.name MR_capitalizedFirstCharacterString]]);
                                  BOOL implementsShouldImport = (BOOL) [self respondsToSelector:shouldImportSelector];
                                  if (![relationshipInfo isToMany]) {
                                      if (!(implementsShouldImport && !(BOOL) [self performSelector:shouldImportSelector
                                                                                         withObject:localObjectData])) {
                                          NSManagedObject *relatedObject = [weakself MR_findObjectForRelationship:relationshipInfo
                                      ...
                                      for (id singleLocalObjectData in localObjectData) {
                                          if (!(implementsShouldImport && !(BOOL) [self performSelector:shouldImportSelector
                                                                                             withObject:singleLocalObjectData])) {
                                      ...
                                      if ([localObjectDataToImport count] > 0) {
                                          ...
                                          NSArray *result = [NSClassFromString(relationshipInfo.destinationEntity.name) MR_importFromArray:localObjectData
                                                                                                                      withPrimaryAttribute:primaryKeyAttribute
...
+ (id)MR_importFromObject:(id)objectData inContext:(NSManagedObjectContext *)context; {
    ...
    NSManagedObject *managedObject = [selfClass MR_findFirstByAttribute:[primaryAttribute name]
                                                              withValue:value
                                                              inContext:context];
...
+ (NSDictionary *)MR_cachedObjectsForArray:(NSArray *)listOfObjectData
    ...
    if (entity.subentities.count) {
        ...
        NSMutableDictionary *substitutionVariables = [NSMutableDictionary dictionary];
        ...
        NSMutableArray *keysToFetchBy = [NSMutableArray array];
        ...
        compoundPredicate = [NSPredicate predicateWithFormat:@"%K in %@", primaryAttribute.name, keysToFetchBy];
    }
...
+ (NSArray *)MR_importFromArray:(NSArray *)listOfObjectData
    ...
    NSDictionary *objectCache = [self MR_cachedObjectsForArray:listOfObjectData
                                          withPrimaryAttribute:primaryAttribute
                                                     inContext:context];
    for (id singleObjectData in listOfObjectData) {
