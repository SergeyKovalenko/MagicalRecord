...
testMappedEntity.
testMappedEntityIDValue = 42;
testMappedEntity.
sampleAttribute = @"This attribute created as part of the test case setup";
...
expect(testRelatedEntity)
.toNot.
beNil();
expect([
testRelatedEntity sampleBaseAttribute
]).to.contain(@"BASE");
expect([
testRelatedEntity respondsToSelector:
@selector(sampleConcreteAttribute)]).to.
beFalsy();
});

it(@"via a 'to many' relationship", ^{
expect(singleTestEntity
.testAbstractToManyRelationship).to.haveCountOf(2);
...
expect([
testRelatedEntity sampleBaseAttribute
]).to.contain(@"DESCENDANT");
});

it(@"via a 'to many' relationship", ^{
expect(singleTestEntity
.testConcreteToManyRelationship).to.haveCountOf(3);
