...
[
managedObjectContext obtainPermanentIDsForObjects:
@[inserted]
error:nil];
...
expect(rootFetchedObject)
.to.
beNil();
expect([
rootFetchedObject hasChanges
]).to.
beFalsy();
...
[
managedObjectContext MR_saveOnlySelfWithCompletion:
^(
...
expect(completionBlockCalled)
.will.
beTruthy();
expect(completionBlockIsOnMainThread)
.will.
beTruthy();
});
...
expect(fetchedObject)
.will.
beNil();
expect([
fetchedObject hasChanges
]).will.
beFalsy();
...
NSError *fetchError;
NSManagedObject *fetchedObject = [[NSManagedObjectContext MR_rootSavingContext] existingObjectWithID:objectId
                                                                                               error:&fetchError];
...
[
SingleEntityWithNoRelationships MR_createInContext:
...
[
managedObjectContext MR_saveToPersistentStoreWithCompletion:
^(
BOOL success, NSError
*error) {
completionBlockCalled = YES;
...
it(@"to self only", ^{
[
managedObjectContext MR_saveWithOptions:
MRSaveSynchronously completion:
^(
BOOL success, NSError
*error) {
expect(success)
...
managedObjectContext MR_saveWithOptions:
MRSaveSynchronously |
MRSaveParentContexts completion:
^(
BOOL success, NSError
*error) {
expect(success)
...
NSError *fetchError;
NSManagedObject *fetchedObject = [[NSManagedObjectContext MR_rootSavingContext] existingObjectWithID:permanentObjectID
                                                                                               error:&fetchError];
